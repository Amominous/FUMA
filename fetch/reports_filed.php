...
foreach($result as $row)
{
	...
	if (!isset($_GET["status"]))
	{
		$name = $row['type'] == 'Student' ? $row['student_name'] : $row['teacher_name'];
		$sub_array[] = "<b>By:</b> ".$name."<br>(".($row['type'] == 'Student' ? $row['type'] : 'Instructor').")"
	...
	else
	{
		if (!empty($row["image"]))
		{
			$sub_array[] = '<a data-magnify="gallery" class="card-img-top " data-caption="'.($row["pc_no"].' - '.$row["pc_status"]).'" data-group="" href="'.$row["image"].'">
		...
		$name = $row['type'] == 'Student' ? $row['student_name'] : $row['teacher_name'];
		$sub_array[] = $name."<br>(".($row['type'] == 'Student' ? $row['type'] : 'Instructor').")";
